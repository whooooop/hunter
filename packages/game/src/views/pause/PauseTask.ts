...
export class PauseTask extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, title: string, number: number, done: boolean, stars: number) {
    ...
    const numberText = scene.add.text(-170, 0, number.toString(), { fontSize: 24, color: '#fff', fontFamily: FONT_FAMILY.REGULAR, stroke: '#000', strokeThickness: 3 }).setOrigin(0.5);
    const text = scene.add.text(-130, 0, title.toUpperCase(), { fontSize: 20, color: '#fff', fontFamily: FONT_FAMILY.REGULAR, stroke: '#000', strokeThickness: 3 }).setOrigin(0, 0.5).setWordWrapWidth(270);
